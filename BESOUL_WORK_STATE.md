...
## HOTFIX-FINANZAS-CENTROS-TARIFAS — Alfa Prime + tarifa especial Verónica (2026-09-03)

Rama `HOTFIX-FINANZAS-CENTROS-TARIFAS`, desde `main` (`1c1a740`, ya incluye `BESOUL-NEXT-UX-NOTIFICATIONS` mergeado). **Sin merge todavía.**

**Bug 1 — Centro (commit `f4e0606`)**: NO era el bug de escritura ya corregido la sesión anterior (`guardarConfigFinanzas()` ya envía el objeto completo sin claves de punto). Causa real: `renderConfigCentros()`/`renderConfigTrainers()` mostraban el valor VIVO de un campo temporal (`canonPt`, `canonFijo`, `canonFijoIndependiente`, `sistemaPago`, `centroId`...) sin resolver la vigencia. Si el admin elegía "Aceptar" (= solo este mes) en el diálogo de vigencia, `registrarVersionTemporal()` correctamente NO toca el valor vivo por diseño — pero el panel seguía enseñando el valor antiguo, pareciendo un guardado fallido aunque se había guardado bien como excepción de ese mes. Corregido resolviendo ambos paneles contra el mes de trabajo actual (`centroParaMes()`/`settingsTrainerParaMes()`, las mismas funciones que ya usa el cálculo real) antes de renderizar. Cero cambios en el modelo de escritura/temporalidad. Verificados todos los campos editables de centro (numéricos, booleanos, texto) — el resto de campos no son temporales y no tienen este problema.

**Bug 2 — Tarifa especial (commit `d3d0c7b`)**: motor de cálculo verificado correcto mediante trazado manual completo con un caso real de la especificación original (Pilates Máquina general 8 sesiones → 95€ → 47.50/33.25/14.25€). Confirmado que los `<select>` en cascada de `agenda.html` (actividad/modalidad/segmento/plan) siempre toman sus valores directamente de las claves del propio catálogo (`Object.keys(...)`, `plan.sesiones`) — no hay forma de que un valor guardado en la ficha no coincida con el catálogo por error de tecleo. **Paridad Finanzas/Dashboard confirmada intacta** (diff función por función, idénticas salvo formato/whitespace y dos diferencias puramente cosméticas función-inline-vs-helper con el mismo resultado) — no hizo falta eliminar ninguna divergencia. Añadido logging `[FINANZAS][TARIFA_NO_ENCONTRADA]` (trainerKey/centerKey/activityKey/modalityKey/segmentKey/sessions/mesKey, sin PII) en el único punto donde antes se devolvía 0 en silencio, en ambos archivos. **Explicación más probable del 0 real reportado, no un bug de código**: si la ficha de Verónica se configuró con la actividad especial ANTES del commit `7a589c4` (el fix de persistencia de guardado de clientes de la sesión anterior), ese cambio pudo no haber llegado nunca a Firestore — el fix arregla los guardados futuros, no recupera retroactivamente datos que nunca se persistieron. Reabrir y volver a guardar su ficha ahora debería resolverlo; si el problema es un desajuste real de catálogo, el nuevo log lo señalará exactamente en la consola.

**Sin cambios de arquitectura de negocio, sin Rules, sin datos reales tocados/creados.**
