...
1. Verónica Dell'Agnese + catálogo de tarifas (Pilates Máquina / Ciclo Indoor) + reparto porcentual configurable 50/35/15 (FASE 9-13) — es el siguiente bloque de prioridad alta, funcionalidad nueva grande, tratar como su propio bloque de trabajo. Primer paso real: confirmar si el perfil de Verónica ya existe en `besoulUsers` o si hay que crearlo con `crearEntrenador()` (ya protegido con `normalizarTrainerKey()`); no inventar su `trainerKey`.
2. Diseñar el catálogo de tarifas como datos en Firestore (no hardcodear en JS), reutilizando el mismo patrón de "versiones por mes" ya construido en FASE 6-7 si el negocio necesita que las tarifas de Pilates/Ciclo también cambien sin afectar a meses pasados.
3. Miguel Fenech: cerrado y documentado arriba, no requiere ninguna acción de continuación salvo que el usuario reporte una recurrencia del problema.
4. Herramienta de auditoría "matriz trainerKey" (solo lectura) sigue pendiente si se quiere detectar mismatches similares a Miguel Fenech en otros PT — no bloqueante, hacer si hay hueco antes de FASE 9-13.
